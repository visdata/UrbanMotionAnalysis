...
filelist  = ['P2-part-'+format(n, '05d')+'-trajectory_30-800' for n in range(1000)]

input_path = '/datahouse/tripflow/labelData-30-800-TJ/'
...
def iterateFile(filename):
    ...
    for record in records:
        ...
        if ydayCurrent < 0 or ydayCurrent >= 87 or gid >= 104000 or gid < 0:
            continue
        hourSeg = ydayCurrent * 24 + hour

        key = str(hourSeg)+'_'+str(gid)+'_'+str(uid)
        ...
        if stat == 1 and not travel_hour_gid_uid_dict.has_key(key):
            res[hourSeg][gid][2] += 1
