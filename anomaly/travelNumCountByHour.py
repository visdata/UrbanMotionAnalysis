...
def processTask(day, stdindir):
    ...
    for index in xrange(0, 24):
        if day * 24 + index > MAXHOUR:
            break;
        count = 0;
        with open(os.path.join(stdindir, 'traveldata-%d') % (day * 24 + index), 'r') as dinput:
...
def main(argv):
    try:
        ...
        str(err);
        sys.exit(2);
    stdindir = "/datahouse/tripflow/2019-30-800-TJ/tj-byhour-tf";
    stdoutdir = "/datahouse/tripflow/Anomaly/tj-flowcount";
    day = 87;
    ...
    with open(os.path.join(stdoutdir, "travelcount_tj.json"), 'wb') as output:
