...
count_zero = [[0, 0, 0] for i in range(104000)];
...
def main():
    ...
    for file_index in range(startIndex, endIndex + 1):
        ...
        for record_index in range(len(records_array[file_index])):
            ...
            raw_value_vector = np.array(raw_value_list, dtype=np.float64)
            if df1[file_index / 24][record_index][3] != 0 and count_zero[record_index][2] < endIndex * zero_rate:
            ...
            if df2[file_index % 24][record_index][3] != 0 and count_zero[record_index][2] < endIndex * zero_rate:
