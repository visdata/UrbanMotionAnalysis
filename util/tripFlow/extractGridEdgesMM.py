...
from util.tripFlow.mapMatchingGridEdges import MapMatchingGridEdges
...
class ExtractGridEdgesMM(object):
	def __init__(self, PROP):
		...
		self.city = PROP['city']
		self.INPUT_PATH = os.path.join(PROP['IDIRECTORY'], self.city.lower()+'-byhour-tf')
		self.OUTPUT_PATH = os.path.join(PROP['ODIRECTORY'], self.city.lower()+'-byhour-rec-mapped')
	...
	def updateResByLine(self, fPoint, tPoint, fromGid, toGid, direction, speed, type='all'):
		...
		if type == 'all' or type == 'from':
			...
			fromCVecStr = "%s,%d,from,%f,%s,%.1f,1" % (fPointStr, fromGid, speed, direction, fangle)
			...
			if self.delta > 0:
				for x in xrange(-2, 3):
					for y in xrange(-2, 3):
						...
						newGID = getGIDByIndex(fromGid, x, y, self.LngSPLIT, self.LatSPLIT, self.locs)
						newStrength = pow(math.e, -(x*x+y*y)/self.delta)
						fromCVecStr = "%s,%d,from,%f,%s,%.1f,%f" % (fPointStr, newGID, speed, direction, fangle, newStrength)
			...
			if self.delta > 0:
				for x in xrange(-2, 3):
					for y in xrange(-2, 3):
						...
						newGID = getGIDByIndex(toGid, x, y, self.LngSPLIT, self.LatSPLIT, self.locs)
						newStrength = pow(math.e, -(x*x+y*y)/self.delta)
						toCVecStr = "%s,%d,to,%f,%s,%.1f,%f" % (tPointStr, newGID, speed, direction, tangle, newStrength)
