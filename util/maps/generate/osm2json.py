...
def process_element(fnode, fway, elem):
...
def fast_iter(context, maxline, fnode, fway):
	placement = 0
	try:
		for _, elem in context:
			
			process_element(fnode, fway, elem)  #处理每一个元素,调用process_element.    
			placement += 1
...
def main(argv):
	...
	fast_iter(context, maxline, fnode, fway)
